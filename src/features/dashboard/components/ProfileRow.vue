...
<template>
  <div class="profile-row" :class="{ selected: isSelected, running: profile.status === 'running' }"
    @click="emit('select', profile)">
    <!-- 选择框 -->
    <div class="col-checkbox" @click.stop>
      <input type="checkbox" :checked="isSelected" class="checkbox-input" @change="emit('select', profile)" />
    </div>
    ...
    <div class="col-name">
      <div class="name-cell">
        ...
        <div class="name-info">
          ...
          <p class="profile-id" @click.stop="copyId" title="点击复制完整 ID">ID: {{ profile.id.slice(0, 4) }}...{{
            profile.id.slice(-4) }}</p>
        </div>
      </div>
    </div>
    ...
    <div class="col-proxy">
      <div class="proxy-info">
        ...
        <span class="proxy-detail" v-if="profile.proxy">{{ profile.proxy.type?.toUpperCase() || 'SOCKS5' }} · {{
          profile.proxy.port }}</span>
      </div>
    </div>
    ...
    <div class="col-platform">
      <!-- Windows 图标 -->
      <svg v-if="getSystemPlatform(profile).type === 'windows'" class="platform-icon" fill="currentColor"
        viewBox="0 0 24 24">
        <path
      ...
      <!-- macOS 图标 -->
      <svg v-else-if="getSystemPlatform(profile).type === 'macos'" class="platform-icon" fill="currentColor"
        viewBox="0 0 24 24">
        <path
      ...
      <!-- Android 图标 -->
      <svg v-else-if="getSystemPlatform(profile).type === 'android'" class="platform-icon" fill="currentColor"
        viewBox="0 0 24 24">
        <path
      ...
      <!-- iOS 图标 -->
      <svg v-else-if="getSystemPlatform(profile).type === 'ios'" class="platform-icon" fill="currentColor"
        viewBox="0 0 24 24">
        <path
          d="M18.71 19.5c-.83 1.24-1.71 2.45-3.1 2.48-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.31-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.36 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z" />
      </svg>
    ...
    <div class="col-actions" @click.stop>
      <!-- 启动/停止按钮 -->
      <button v-if="profile.status === 'running'" class="action-btn action-stop" title="停止"
        @click="handleAction('stop')">
        <span class="material-symbols-outlined">stop_circle</span>
      </button>
      <button v-else class="action-btn action-start" title="启动" @click="handleAction('launch')">
